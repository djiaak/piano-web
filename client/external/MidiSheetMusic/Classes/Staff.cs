...
namespace MidiSheetMusic
{
    ...
    public class Staff
    {
        ...
        public void Draw(Graphics g, Rectangle clip, Pen pen, int selectionStartPulse, int selectionEndPulse, Brush deselectedBrush)
        {
            /* Shade any deselected areas */
            ShadeSelectionBackground(g, clip, selectionStartPulse, selectionEndPulse, deselectedBrush);
        ...
        */
        ...
        {
            if (selectionEndPulse == 0) return;

            int xpos = keysigWidth;
            bool lastStateFill = false;
            foreach (MusicSymbol s in symbols)
            {
                if (InsideClipping(clip, xpos, s) && (s.StartTime < selectionStartPulse || s.StartTime > selectionEndPulse))
                {
                    g.TranslateTransform(xpos - 2, -2);
                    g.FillRectangle(deselectedBrush, 0, 0, s.Width + 4, this.Height + 4);
                    g.TranslateTransform(-(xpos - 2), 2);
                    lastStateFill = true;
                }
                else
            ...
            if (lastStateFill)
            {
                ...
                g.TranslateTransform(xpos - 2, -2);
                g.FillRectangle(deselectedBrush, 0, 0, width - xpos, this.Height + 4);
                g.TranslateTransform(-(xpos - 2), 2);
            }
        }
