...
namespace MidiSheetMusic
{
    ...
    public class MidiFile
    {
        ...
        private byte[] ParseRiffData(byte[] data)
        {
            var riffFile = RiffParser.ParseByteArray(data);
            if (riffFile.FileInfo.FileType != "RMID")
            {
                return data;
            }
            while (riffFile.Read((type, isList, chunk) =>
            {
                if (!isList && type.ToLower() == "data")
                {
                    data = chunk.GetData();
                }
            })) ;
        ...
        {
            string header;
            if (RiffParser.IsRiffFile(data, out header))
            {
                data = ParseRiffData(data);
            }
