...
namespace MidiSheetMusic
{
    ...
    {
        private static int noteIdCounter = 0; /** Counter used to generate unique note IDs */
        ...
        public MidiTrack(List<MidiEvent> events, int tracknum)
        {
            ...
            foreach (MidiEvent mevent in events)
            {
                ...
                {
                    MidiNote note = new MidiNote(noteIdCounter++, mevent.StartTime, mevent.Channel, mevent.Notenumber, 0, mevent.Velocity);
                    AddNote(note);
                }
